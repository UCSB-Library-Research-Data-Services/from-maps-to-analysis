...
![](images/usa-topomaps.png){width="90%" fig-align="center"}
...
![](images/adding-basemap.png){width="90%" fig-align="center"}
...
![](images/USGS-NLCD-basemap-example.png){width="90%" fig-align="center"}
...
After adding our three layers, we have some work to do to make the map more readable. First, let's adjust the symbology of the parking racks layer to reduce its size and change its symbol to a bike icon.
...
To change the size of the symbol, we can use the `Size` slider to reduce it to a more appropriate size. On this case, we're using 5.68 px. Also, select "Adjust size automatically" if you want the symbols to scale as you zoom in and out of the map.
...
- Select the attribute `b_use`, that stands for building use, and click on `Add`.

![](images/Workshop1/Step2/symbology-by-attribute.png){width="90%" fig-align="center"}

As result, you will see that the buildings are now filled with different colors based on their use. You can further customize the colors by clicking on the color ramp and selecting a different one.
...
As result, you will see that the bike paths are now more transparent for higher speed limits and less transparent for lower speed limits.
